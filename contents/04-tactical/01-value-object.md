...
このリポジトリでは、検証用コードの `src/domain/task.rs` で次のように `TaskTitle` を定義しています。
...
`cargo test` を実行すると、`src/lib.rs` のテストで `TaskTitle` の不変条件を確認できます。たとえば、空文字や101文字のタイトルが拒否されることを検証しています。
